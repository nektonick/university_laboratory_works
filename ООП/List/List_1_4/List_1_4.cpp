...
template <class T1>
class IteratedLinkedList : public LinkedList<T1> {
protected:
public:
	IteratedLinkedList() : LinkedList<T1>() {
		cout << "\nIteratedLinkedList constructor";
	}
	virtual ~IteratedLinkedList() {
		cout << "\nIteratedLinkedList destructor";
	}
	ListIterator<T1> begin() const{
		ListIterator<T1> it = LinkedList<T1>::head;
		return it;
	}
	ListIterator<T1> end() const{
		ListIterator<T1> it = LinkedList<T1>::tail->next;
		return it;
	}
	Element<T1>* pop() {
		if (this->isEmpty()) {
...
int main()
{
	...
	IteratedLinkedList<Patient> iter_list;
	iter_list.push(Patient("Сидоров"));
	iter_list.push(Patient("АААА", "FFF", Date(), "1", "", 100, 3));
	cout << "\n~~~Iterated print:~~~\n"; iter_list.print();

	cout << "\n~~~Iterated print 2:~~~\n";
	ListIterator<Patient> iterator = iter_list.begin();
	while (iterator != iter_list.end()) {
